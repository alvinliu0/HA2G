...
def generate_gestures_hierarchy(args, g1, g2, g3, audio_encoder, lang_model, audio, words, 
                    target_1, target_2, target_3, audio_sr=16000, vid=None, fade_out=False):
    ...
    spectrogram_sample_length = utils.data_utils.calc_spectrogram_length_from_motion_length(args.n_poses, args.motion_resampling_framerate)
    ...
    for i in range(0, num_subdivision):
        ...
        if i > 0:
            ...
            target_1[:, 0:args.n_pre_poses, :] = torch.cat((out_dir_vec[:, -args.n_pre_poses:, :4 * 3], out_dir_vec[:, -args.n_pre_poses:, 6 * 3:7 * 3]), dim = 2)
            target_2[:, 0:args.n_pre_poses, :] = torch.cat((out_dir_vec[:, -args.n_pre_poses:, :5 * 3], out_dir_vec[:, -args.n_pre_poses:, 6 * 3:8 * 3]), dim = 2)
            target_3[:, 0:args.n_pre_poses, :] = out_dir_vec[:, -args.n_pre_poses:, :]
        ...
        out_dir_vec_1, *_ = g1(pre_seq_1.float(), in_text_padded, linear_blend_feat[0], vid)
        ...
        out_dir_vec, *_ = g3(pre_seq_3.float(), in_text_padded, linear_blend_feat[2], vid)  # out shape (batch x seq x dim)
...
def main(mode, checkpoint_path, option):
    args, g1, g2, g3, audio_encoder, loss_fn, lang_model, speaker_model, pose_dim = utils.train_utils.load_checkpoint_hierarchy(
    ...
    # load lang_model
    vocab_cache_path = os.path.join('/mnt/lustressd/share/liuxian.vendor/ted_dataset', 'vocab_cache.pkl')
    ...
    if mode == 'eval':
        ...
        evaluate_testset(data_loader, g1, g1, g2, g3, audio_encoder, loss_fn, embed_space_evaluator, args)
        ...
        tmp_target_dir_vec = torch.zeros((1, 4, 27)).to(device)
        target_1 = torch.cat((tmp_target_dir_vec[:, :, :4 * 3], tmp_target_dir_vec[:, :, 6 * 3:7 * 3]), dim = 2)
        target_2 = torch.cat((tmp_target_dir_vec[:, :, :5 * 3], tmp_target_dir_vec[:, :, 6 * 3:8 * 3]), dim = 2)
        target_3 = tmp_target_dir_vec

        dir_vec = generate_gestures_hierarchy(args, g1, g2, g3, audio_encoder, lang_model, audio, words_with_timestamps, 
                target_1, target_2, target_3, vid=vid, fade_out=False)
        ...
        with lmdb_env.begin(write=False) as txn:
            ...
            while n_saved < n_generations:  # loop until we get the desired number of results
                ...
                target_dir_vec = torch.from_numpy(target_dir_vec.reshape(target_dir_vec.shape[0], -1))
                ...
                out_dir_vec = generate_gestures_hierarchy(args, g1, g2, g3, audio_encoder, lang_model, clip_audio, clip_words, 
                    target_1, target_2, target_3, vid=vid_idx, fade_out=False)
